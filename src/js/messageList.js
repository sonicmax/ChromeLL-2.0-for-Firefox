var CHROMELL = (function() {
	var messageList = {};
	
	messageList.init = function(config) {
	...
	messageList.DOM = function() {
		...
		// NOTE: The ollowing methods are only called for new livelinks posts:
			...
			quickreply
					.addEventListener(
							...
							function(evt) {
								...
								allPages.commonFunctions.asyncUpload(evt.dataTransfer.files);
							});
		};
	...
	messageList.eventHandlers = function() {
		...
		handlers.mouseenter = function(evt) {
			if (evt.target.className == 'like_button' && this.config.custom_like_button) {
				...
				popupDebouncer = setTimeout(allPages.popup.handler.bind(allPages.popup), 750);
			}
		};
	...
	messageList.utils = function() {
		...
		utils.gfycat = function() {
			// Contains methods for Gfycat video embedding.
			
			var checkAPI = function (url, callback) {
		...
		utils.anchors = function() {			
			...
			var handleGfycat = function(link) {
		...
		return {
			startBatchUpload: function(evt) {
				...
				document.getElementsByClassName('quickpost-body')[0]
						.getElementsByTagName('b')[0].innerHTML += " (Uploading: 1/"
						+ chosen.files.length + ")";
				allPages.commonFunctions.asyncUpload(chosen.files, 0);
			},		
