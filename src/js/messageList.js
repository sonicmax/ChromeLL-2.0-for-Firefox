...
var messageList = {
	functions: {
		posts: {
			...
			autoscroll: function(mutation, index, live) {
				if (live) {
					if (document.hidden 
							&& messageList.helpers.autoscrollCheck(mutation) ) {
		...
		page: {
			...
			expand_spoilers: function() {
				...
				infobar.appendChild(divider);
				infobar.appendChild(anchor);
				anchor.addEventListener('click', messageList.helpers.spoilers.find);		
			},
					...
					function(event) {
						if (config.snippet_alt_key) {
							if (event.shiftKey == true
								...
								caret = messageList.helpers.snippets.findCaret(ta);
								messageList.helpers.snippets.handler(ta.value, caret);				
							}
						}
	...
	helpers: {
		gfycat: {
					...
					if (xhr.readyState == 4 && xhr.status == 200) {
						...
						if (gfyLink.parentNode) {
							...
							if (position.top > window.innerHeight) {
								...
								messageList.helpers.gfycat.embed(placeholder);
							}
						}
					}
				}
			...
			pause: function() {
				...
				if (document.hidden) {
					...
					messageList.helpers.gfycat.loader();
				}
			}
		},
		youtube: {
		...
		bash: {
			...
			checkSelection: function(target) {
				...
				if (popup) {
					if (len > 1) {
						...
						messageList.helperss.bash.reset(bashes);
					}
				}
			},
			reset: function(bashes) {
				...
				messageList.helpers.bash.closePopup();
			},
			closePopup: function() {
			...
			handler: function() {
				...
				for (var i = 0, len = bashes.length; i < len; i++) {
					...
					if (quotes.length > 0) {
						...
						message = messageList.helperss.bash.getMessage(message_to_quote, quotes, tops, -1);
						if (!message) {
							// do nothing
						}
						else {
							username = messageList.helperss.bash.getUsername(top);
							user_array.push(username);
							message_array.push(message);	
						}
					}
					...
					if (i === 0) {
						...
						url = messageList.helperss.bash.getURL(first_top, len);
					}
				}
				// pass values to submitBash function
				messageList.helperss.bash.submit(url, user_array, message_array)
		...
		quotes: {
			handler: function() {
				...
				for (var i = 0, len = nodes.length; i < len; i++) {
					...
					if (node.className == 'spoiler_closed') {
						...
						output += messageList.helperss.quotes.returnSpoiler(spoiler.caption, spoiler.nodes);
					}	
					if (node.className == 'quoted-message') {
						...
						if (quote.nested.length > 0) {
							...
							for (var m = quote.nested.length; m--;) {
								...
								quoteArray = messageList.helpers.quotes.returnQuotes(nestedQuote.childNodes, 
										nestedQuote.attributes.msgid.value);
								quoteOutput = quoteArray[0] + quoteOutput + quoteArray[1];
							}
							quoteArray = messageList.helpers.quotes.returnQuotes(node.childNodes, quote.msgid);
							quoteOutput = quoteArray[0] + quoteOutput + quoteArray[1];
							output += quoteOutput;
						}
						else {
				...
				// alert user
				messageList.helpers.quotes.notify(_this);
			},
			...
			addButtons: function() {
				...
				$("div.message-top").on("click", "a.archivequote", messageList.helpers.quotes.handler);
			}
		},
		images: {
			expand: function(evt) {
		...
		spoilers: {
			find: function(el) {
				...
				for (var i = 0; spans[i]; i++) {
					...
					messageList.helpers.spoilers.toggle(node);
				}
			},
			toggle: function(obj) {
		...
		links: {
			check: function(msg) {
				...
				if (config.embed_gfycat || config.embed_gfycat_thumbs) {
					messageList.helpers.gfycat.loader();
					window.addEventListener('scroll', messageList.helpers.gfycat.loader);
					document.addEventListener('visibilitychange', messageList.helpers.gfycat.pause);
				}
			},	
	...
	addListeners: function() {
		...
		body.addEventListener('click', function(ev) {
			if (config.user_notes) {
				messageList.helpers.usernotes.open(ev.target);
			}
			if (config.post_templates) {
				messageList.helpers.postTemplateAction(ev.target);
			}
			if (ev.target.title.indexOf("/index.php") == 0) {
				// TODO - this should point to links.fix
				messageList.helpers.links.wiki(ev.target);
				ev.preventDefault();
			}
			else if (ev.target.title.indexOf("/imap/") == 0) {
				// TODO - this should point to links.fix
				messageList.helpers.links.imagemap(ev.target);					
				ev.preventDefault();
			}
			else if (ev.target.parentNode.className == 'embed') {
				messageList.helpers.youtube.embed(ev.target.parentNode);
			}
			else if (ev.target.parentNode.className == 'hide') {
				messageList.helpers.youtube.hide(ev.target.parentNode);			
			}
				...
				ev.target.innerHTML = '&#9745;';
				messageList.helpers.bash.checkSelection(ev.target);
				messageList.helpers.bash.showPopup();		
			}
				...
				messageList.helpers.bash.handler();
			}
		});
	},
	...
	callFunctions: function(pm) {
		...
		var msg, len;
		for (var k in messageList.functions.page) {
			if (config[k + pm]) {
					messageList.functions.page[k]();
			}
		}
		...
		for (var j = 0; j < len; j++) {
			...
			for (var k in messageList.functions.posts) {
				if (config[k + pm]) {
						// pass msg and index value to function
						messageList.functions.posts[k](msg, j);
				}
			}
		}
		...
		for (var i in messageList.functions.misc) {
			if (config[i + pm]) {
				messageList.functions.misc[i]();
			}
		}
		// call any functions that don't exist in messageList object
		meessageList.helpers.quotes.addButtons();
		messageList.helpers.links.check();
	...
	livelinks: function(mutation) {
		...
		messageList.helpers.links.check(mutation);
		// send ignorator data to background script
		messageList.globalPort.postMessage({
			action : 'ignorator_update',
			ignorator : ignorated,
