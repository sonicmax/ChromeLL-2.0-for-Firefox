document
		...
		.addEventListener(
				...
				function() {
					...
					qp(
							data.text,
							document.getElementById('post_action').parentNode.parentNode);
...
function qp(title, element) {
	...
	var uib = element.parentNode.parentNode.getElementsByTagName('a').item(0).href
			.indexOf('user=');
	var uid = element.parentNode.parentNode.getElementsByTagName('a').item(0).href;
	var ui = uid.substring(uib + 5, uid.length);
	var u = document.getElementsByClassName('userbar').item(0)
			.getElementsByTagName('a').item(0).innerHTML.split('(')[0];
	...
	title = title.replace('%f', f).replace(/%u/g, u).replace('&lt;', '<')
			.replace('&gt;', '>').replace(/%i/ig, ui)
			+ sig;
	if (title.indexOf('%q') == -1) {
		...
		document.getElementsByName('message')[0].value = title.replace('%q ',
				'').replace('%q', '');
		QuickPost.publish('quote', element.parentNode);
	}
}